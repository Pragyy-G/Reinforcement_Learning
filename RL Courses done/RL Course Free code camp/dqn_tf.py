...
class DeepQNetwork(object):
    def __init__(self,lr, n_actions, name, fcl_dims=256,
                 input_dims=(210,160,4), chkpt_dir='tmp/dir'):
        self.lr = lr
        self.name= name
    ...
    def build_network(self):
        ...
        cov1 = tf.layers.conv2d(inputs=self.input, filters=32, kernel_size=(8,8), strides=4,name='conv1', 
                               kernel_initializer=self.variance_scaling_initializer(scale=2))
        conv1_activated = tf.nn.relu(conv1)
        
        cov2 = tf.layers.conv2d(inputs=self.input, filters=64, kernel_size=(4,4), strides=2,name='conv2', 
                               kernel_initializer=self.variance_scaling_initializer(scale=2))
        conv2_activated = tf.nn.relu(conv2)

        cov3 = tf.layers.conv2d(inputs=self.input, filters=128, kernel_size=(3,3), strides=1,name='conv3', 
                               kernel_initializer=self.variance_scaling_initializer(scale=2))
...
class Agent(object):
    def __init__(self,alpha, gamma, mem_size, n_actions, epsilon, batch_size, replace_target=5000,
        ...
        self.action_memory = np.zeroes(self.mem_size, self.n_actions, dtype = np.int8)
        self.reward_memory = np.zeroes(self.mem_size)
        self.treminal_memory = np.zeroes(self.mem_size, dtype = np.int8)

    def store_trasmittion(self, state, action, reward, sttae_, termina):
            index = self.mem_cntr % self.mem_size
            self.state_memroy[index] = state
            actions = np.zeroes(self.n_actions)
            actions[actions] = 1.0
            self.action_memory[index] = actions
            self.replace_memory[index] = reward
    ...
    def learn(self):
            if self.mem_cntr & self.replace_target_memory ==0:
                self.update_graph()
            ...
            action_indicies = np.dot(action_batch, action_values)
            action_batch = self.reward_memory[batch]
            terminal_batch = self.terminal_memory[batch]

            q_eval = self.q_eval.sess.run(self.q_eval.Q_values,
                                          feed_dict = {self.q_eval.input: state_batch})
            q_next = self.q_next.sess.run(self.q_eval.Q_values,
                                          feed_dict = {self.q_eval.input: new_state_batch})
            

            q_target = q_eval.copy()
            q_target[:, action_indicies] = reward_batch + \
                    self.gamma*np.max(q_next, axis=1)*terminal_batch
            
            _ = self.q_eval.sess.run(self.q_eval.train.op,
                                     feed_dict = {self.q_eval.input:state_batch,
                                                  self.q_eval.actions: action_batch,
                                                  self.q_eval.target:q_target})
